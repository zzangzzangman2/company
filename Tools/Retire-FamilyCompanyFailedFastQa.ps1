...
$failure = Get-Content -LiteralPath (Join-Path $FailureDirectory 'process.json') -Raw | ConvertFrom-Json
if (!$build.passed -or $build.head -cne $ExpectedSourceHead -or $cache.head -cne $ExpectedBaseDataHead -or
    $failure.exitCode -eq 0 -or $cache.output -cne (Join-Path $qaTarget 'FamilyCompany_FastQa.exe')) { throw 'Failed-test/build identity mismatch.' }
...
@{root=$qaTarget;sourceHead=$build.head;sourceTree=$tree;baseDataHead=$cache.head;unityVersion=$build.unityVersion;
    ...
    expected='All required gameplay assertions pass';actualExitCode=$failure.exitCode;rollback='none';files=$files;
