#!/usr/bin/env python3
"""Publish the two exact Player-west contact poses from the canonical 4x2 sheet.

No pixels are generated or interpolated. Approved direction outputs are not
read or modified; west is extracted independently from column one.
"""
...
OUTPUT = REPO / "Assets/Resources/FamilyCompany/PlayerWestContactV1/Frames"
GRID_COLUMNS = 4
GRID_ROWS = 2
WEST_COLUMN = 1
...
def main() -> None:
    ...
    for row in range(GRID_ROWS):
        cell_box = (
            WEST_COLUMN * cell_width,
            row * cell_height,
            (WEST_COLUMN + 1) * cell_width,
            (row + 1) * cell_height,
        )
        cell = sheet.crop(cell_box)
        alpha_box = cell.getchannel("A").getbbox()
        if alpha_box is None:
            raise ValueError(f"west contact row {row} has no opaque pixels")
    ...
    for index, image in enumerate(trimmed):
        ...
        path = OUTPUT / f"player_west_contact_{index}_v1.png"
        canvas.save(path)
        frames.append({
            "phase": index * 0.5,
    ...
    receipt = {
        "contract": "FC-PLAYER-WEST-CONTACT-V1",
    ...
    print(
        "PLAYER_WEST_CONTACT_V1: PASS | frames=2 generatedPixels=false "
