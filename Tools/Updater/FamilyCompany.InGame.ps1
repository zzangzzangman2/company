...
try {
    ...
    try {
        ...
        # Never use $matches: a successful digest regex overwrites automatic $Matches.
        $manifestAssets = @()
        for ($page=1; $page -le 101; $page++) {
            ...
            $assetPage = @($assetPage)
            $manifestAssets += @($assetPage | Where-Object name -CEQ 'family-company-manifest.json')
            if ($assetPage.Count -lt 100) { break }
            if ($page -eq 101) { throw 'Unbounded release assets.' }
        }
        if ($manifestAssets.Count -ne 1 -or $manifestAssets[0].size -gt 4MB -or $manifestAssets[0].digest -notmatch '^sha256:[0-9a-f]{64}$') { throw 'Missing authenticated manifest digest.' }
        $hash = $manifestAssets[0].digest.Substring(7)
        Receive-PatchFile "https://github.com/$script:PatchRepository/releases/download/$($release.tag_name)/family-company-manifest.json" $temporary $manifestAssets[0].size $hash
