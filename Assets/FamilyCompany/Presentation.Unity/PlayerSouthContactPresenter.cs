...
{
    /// <summary>
    /// South-only, source-exact contact baseline. The existing animator remains
    /// authoritative for every non-south direction and non-walking pose.
    /// </summary>
    public sealed class PlayerSouthContactPresenter : MonoBehaviour
    {
        public const int SouthDirection = 0;
        ...
        public void Configure(SpriteRenderer renderer)
        {
            ...
            for (var index = 0; index < _contacts.Length; index++)
            {
                ...
                if (_contacts[index] == null)
                    throw new InvalidOperationException("Missing Player south contact resource " + index);
            }
            _configured = true;
        }
            ...
            bool presentationAway)
        {
            ...
            bool useContact = !presentationAway && !legacyPoseOwnsPresentation &&
                              moving && direction == SouthDirection;
            IsActive = useContact;
