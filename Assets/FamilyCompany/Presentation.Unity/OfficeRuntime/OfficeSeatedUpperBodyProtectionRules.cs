...
{
    /// <summary>
    /// Builds an upper-body redraw from the currently applied seating Sprite. The chair keeps its
    /// complete authored foreground silhouette; this redraw places only the pose-defined torso,
    /// hands and head back above it. Lower-body pixels remain below the chair foreground.
    /// </summary>
    public static class OfficeSeatedUpperBodyProtectionRules
    {
        ...
        public static Sprite CreateUpperBodySprite(Sprite source, int cutoffSourceY)
        {
            ...
            if (source.texture == null)
                throw new InvalidOperationException("The seating Sprite texture is missing.");
            if (source.pixelsPerUnit <= 0f)
                throw new InvalidOperationException("The seating Sprite PPU must be positive.");
