...
- 45°/90° 코너와 135°/180° 급반전은 방향 변경을 이유로 logical root를 강제 정지하지 않는다. 가속 적분이 속도를 연속 변경하고, 표현 방향은 같은 frame의 실제 변위를 즉시 따른다. 막힌 입력과 상호작용 도착의 최종 facing만 변위 없는 짧은 pivot을 사용하며 Pivot/Idle 중에는 보행 거리를 누적하지 않는다.
- `DirectionalSpriteAnimator`는 공용 규칙의 Presentation adapter다. 최종 renderer에 적용된 clip/Sprite/flip과 같은 frame의 실제 변위·속도·resolved direction을 trace하며, 독립 8방향 원화의 `flipX=false`를 소비 경계에서 강제한다. 모든 Actor는 `1.0 unit/s`, 가속 `8.0`, 한 타일 중심 거리와 같은 `OfficeLocomotionGaitRules.DefaultStrideLength=0.99380799`를 공유한다. 따라서 타일마다 오른발·왼발 한 주기를 끝내며 member ID별 보폭·회전시간·방향 허용치는 두지 않는다. 상호작용은 실제 정지, Idle, 목표 facing이 모두 성립한 뒤에만 Performing으로 전환한다.
