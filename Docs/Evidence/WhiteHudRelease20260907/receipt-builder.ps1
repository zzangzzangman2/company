...
$out='Artifacts/WhiteHudRelease20260907/approval'
...
foreach($validation in @('20260907-182133-836','20260907-182145-736')){
...
Record 'native-binding' @{commit=$commit;passed=$true;independent=$true;baselineNativeCommit='8ce7d3ed';originalNative=$native;
 ...
 scope='NOT new native input. Actual previous native four purchases/rotation/overlap gate is bound only to byte-identical shop controller, skin, navigation adapter and grid placement/transaction subtrees. MainNavigation white header and tab presentation changed; fresh full-navigation EventSystem routes and current shop renders checked separately. Normal movement is covered by fresh normal/walk runs, not this binding.'}
...
$updater=Json 'Artifacts/WhiteHudRelease20260907/updater/all.json'
...
$hud='Artifacts/FastQa/UnifiedOfficeHud/20260907-182049'
$uiNav='Artifacts/FastQa/UnifiedOfficeHud/20260907-182106'
...
foreach($log in @('hud-run.log','navigation-run.log')){
 if((Get-Content "Artifacts/WhiteHudRelease20260907/$log" -Raw) -notmatch 'USER_MAIN_AND_SAVES_UNCHANGED; INTERACTIVE_DESKTOP_UNCHANGED'){throw 'HUD user-state guard missing'}
}
if(@(& git diff --name-only c0709823 $commit -- Assets/FamilyCompany/Simulation Assets/FamilyCompany/Save Assets/FamilyCompany/Runtime/Character3D Assets/FamilyCompany/Presentation.Unity/OfficeRuntime Assets/FamilyCompany/Content/Resources/Production3D Tools/Updater/FamilyCompany.InGame.ps1 Tools/Updater/FamilyCompany.Update.ps1 Tools/Updater/FamilyCompany.Restart.ps1).Count){throw 'UI-only retained scope changed'}
Record 'hud-navigation' @{commit=$commit;passed=$true;independent=$true;hudResult=$hudResult;navigationResult=$uiNavResult;
 ...
 retainedGameplaySource='c0709823';nativeMouseInput=$false;userFilesUnchanged=$true;
...
$map=[ordered]@{'opening-four-actors'='normal-runtime';'shop-native-pointer-four-rotations'='native-binding';'normal-walk-tile-centres'='normal-runtime';'walking-visual-foot-slip-grounding'='walk-acceptance';'furniture-avoidance'='normal-runtime';'four-seated-working-directions'='seated-fit';'next-day-four-staggered-arrivals'='normal-runtime';'mute'='normal-runtime';'runtime-exception-zero'='normal-runtime';'updater-regressions'='updater-regressions';'hud-navigation'='hud-navigation'}
...
Record 'release-receipt' @{schemaVersion=1;commit=$commit;productionEligible=$true;userVisualApproval=$true;
 approvalReference='User continuation ㄱ after pulling the company white HUD source/build and explaining it was not yet a public patch, 2026-09-07. Deploy the received HUD as-is; existing approved world/body/pose/monitor appearance retained. No new character promotion or shutdown.';
 ...
 scope='Unified white office HUD publication; no simulation, world, character, furniture or shipping-worker change from public v4. Fresh Release actual gameplay/geometry/walk, explicit scoped old native source binding. Public transfer verified separately after publication.'}
