...
## 2026-09-07 후속: 내부 화면까지 캐주얼 디자인으로 교체 (미배포)

사용자의 전면 재설계 요청으로 상·하단뿐 아니라 회사/인사/사업/연구/투자와 내부 상세 화면,
상점 및 시장 표시 팔레트를 일치시킨다. 정본은 [캐주얼 UI 명세](UI_CASUAL_REDESIGN_2026-09-07.md).
새 소스에서는 아래 장식형 프레임·Maplestory 상세 화면·고정 grid 크기 계약을 사용하지 않는다.
현재 공개 v5와 작업 중 새 UI를 혼동하지 않는다. 새 화풍의 최종 사용자 확인/공개는 아직 없다.
