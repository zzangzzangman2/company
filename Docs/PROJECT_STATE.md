...
- **2026-08-26 3차 판정: 캐릭터가 진행 방향에 등을 지고 걷고 있었다.** 임포트된 바디의 정면은 `+Z`가
  아니라 `-Z`이므로 `Quaternion.LookRotation(delta)`는 몸을 정확히 반대로 돌린다. 확인 방법은 강제 yaw
  스윕(`-family3d-father-v18-motion-yaw-sweep`)이다. 15도 간격 24방향을 촬영하면 카메라가 보는 면이
  yaw 90~255에서 뒤통수, 270~90에서 얼굴로 나온다. 이후 프레임에 이동 방향 화살표를 그려 직선 구간만
  비교해 확정했다. `LookRotation(-delta)`로 고쳤다.
- **이 하나가 사용자가 지적한 세 증상을 모두 설명한다.** 발은 몸 기준 앞으로 쓸어내는데 몸이 뒤로
  가므로 지면을 반대로 긁어 미끄러지고, 뒤로 걷는 보행은 관절이 어색해 `오징어`로 보이며, 방향은 당연히
  맞지 않는다.
- **계측 방법론 교훈.** 발 스윙 축이 로컬 Z라는 것과 GLB 발가락 방향만으로 "정면축 정상"이라 두 번
  판단했으나, 스윙 축은 앞뒤 대칭이라 부호를 구분하지 못하고 발 평균 위치의 좌우 성분은 0.03~0.06으로
  노이즈 수준이다. 렌더된 화면을 각도별로 통제해 보는 것 외에는 판별할 수 없었다. 방향 관련 주장은
  스윕 시트와 화살표 비교로 검증한 뒤에만 기록한다.
