...
## 2026-08-14 / Shared movement-facing integration candidate

- Added pure `OfficeSharedLocomotionRules` and routed Starter Office Animator/Agent movement through it. Moving display direction now follows actual displacement only; collision slide has no semantic-facing hold and every measured moving frame is constrained to `<=22.5°`, dot `>=0.9239`.
- Reversal now decelerates through its last measurable low-speed displacement to a planted pivot, advances through adjacent 45° facings, then accelerates. Blocked input and interaction arrival can turn in place with displacement 0 and walk-phase advance 0. Performing waits for actual stop, Idle, and desired facing.
- Walk phase uses accumulated actual distance and one shared stride value for all 12 catalog characters. Per-Animator stride override is rejected. Existing 12×8×6 PNG, `.meta`, and GUID assets were not modified.
- Strict pure QA passed: 12 characters, 288 direction checks, 1,970 moving frames, max error `22.4990°`, min dot `0.923886`, 96 reversals with `reverseFacingFrames=0` and `movingDuringPivot=0`, explicit below-walk-threshold final-deceleration and small-corner dead-zone boundaries, 96 blocked pivots, 96 collision slides, 27 FPS/speed/time-scale gait partitions, closure error `0.00000048`, steady-state managed heap growth `0` over 10,000 steps.
- Existing long-straight semantic cursor fix from `0e2965e` was verified without duplicate patch; `OFFICE_PATH_PROGRESS_EXTERNAL: PASS`. Unity 6000.3.21f1 Bee Simulation/Presentation/Editor compilation and strict external harness pass.
- Actor collision remains one ground-position circle (`DefaultRadius=0.22`) passed unchanged through path planning, segment collision and the authored furniture subcell profiles/clearance padding; furniture visual footprint↔semantic footprint QA remains the envelope authority. No sprite root, furniture anchor, collision profile, seating or depth asset was changed here.
- Unity 6000.3.21f1 Editor navigation validation passed with 1,152 paths, 2,289 oracle-matched segments, 12 counterexamples, 170 deadlock ticks, 19 scene obstacles and 27 scene paths. The same final source passed the standalone strict harness and strict walk-art contract (`12` characters, `96/96` loops, `576/576` frames); walk PNG, `.meta`, and GUID hashes remain identical to base.
- Furniture-envelope Editor QA passed all `10,368` cases across `12` profiles and `288` subcells with `78` default-radius clearances and max stop variance `0.00013`. Visual review covered the D3D11 office/work captures and all furniture 8-direction envelope boards.
- Windows x64 build from Unity 6000.3.21f1 passed with input fingerprint `CAECA5FB31ABDFEEE3006AE6FD30D9BA04A15196D4B9E278A0AD8FBC22EC119B`. D3D11 player movement QA passed all 8 directions, a planted South→North reversal, corridor/corner traffic and three desk/counter/NPC collision scenarios with reverse-facing frames `0`, pivot translation `0`, and frame GC allocation max `0 B`.
- A repeated Windows comparison isolated a render-order clock boundary: base `7954d42` passed, while one candidate run consumed a 3.2 s frame on the exact frame that warm-rebind loading cleared and advanced authoritative minutes `324→327`. The realtime source now remembers that presentation was loading and discards the single boundary delta after loading clears; it does not restore a snapshot, alter saved time, or change pause/world-scale semantics. The rebuilt D3D11 extended player passed a 5.68 s warm rebind with `loadingClockDelta=0` and contract save/load minute unchanged. Main integration/deploy/push remain intentionally pending commander approval; this isolated worktree owns only the candidate commit.
- Seating integration handoff remains: the concurrent seating branch owns seat facing lock/phase depth and overlaps Animator/Agent. Merge must preserve shared actual-motion locomotion outside seated phases and resolve the seat-locked `LeavingSeat` exception without duplicating seat state here.

## 2026-08-14 / Four-family seating, occupied-chair depth, and safe egress candidate complete

- The four-family seating lifecycle now keeps the assigned facing locked from `SitDown` through `LeavingSeat`, advances chair/actor presentation with sub-pixel steps, and releases the chair foreground, seat claim, and egress reservation together only after reaching a collision-cleared safe anchor. Front egress is preferred, then left/right; rear exit is not selected.
- Occupied chair compositing has one canonical rule owner: `OfficeSeatedUpperBodyProtectionRules`. It coordinates the unchanged full chair foreground, a 1,816-opaque-pixel lower-body seat-rim crop, and a pose-pelvis upper-body redraw. Removing the lower crop caused a D3D11 father-typing counterexample with zero lower-body/chair overlap, while omitting the upper redraw clipped the mother's torso, so both planes remain parts of one non-duplicated three-layer contract.
- The public runtime limits are now enforced exactly by player QA: seat/egress residual `<=0.9px` and typing hand-to-keyboard residual `<=3.5px`. Generated motion uses `0.899px`/`3.499px` internal budgets to keep floating-point projection roundoff below those public boundaries.
- Unity `6000.3.21f1` static validation passes the 56 approved seat profiles, 24/24 typing frames, four-family foot grounding, 64 egress cases, and 120 continuous-depth permutations. Windows D3D11 player QA passes 56/56 closeups with direction mismatch 0, older-sister 90-degree pop 0, penetration 0, upper-body foreground overlap 0, and typing-hand foreground overlap 0. The normal D3D11 Starter Office main flow and furniture-depth player pass; the Windows build reports warnings 0.
- Final logs and captures are indexed in `Docs/FAMILY_COMPANY_SEATING_TRANSITIONS_DEPTH_CANDIDATE_2026-08-14.md`. Movement candidate `5ba3971` was not cherry-picked; its shared actual-motion work requires a manual semantic merge in `DirectionalSpriteAnimator`, `OfficeRuntimeAgent`, and `OfficeRuntimeOccupancy` after this candidate.
...
## 2026-08-14 / 월드 렌더 선명도·픽셀 안정성 준비

- 월드 카메라의 고정 `960×540` 중간 버퍼 후 Point 확대가 1920×1080에서 바닥·가구·얼굴을
  2×2 블록으로 합친 주원인임을 재현했다. Win32 실측은 1920×1080 borderless, DPI 96,
  PerMonitorV2라 Windows DPI 이중 확대는 원인이 아니다.
- 새 `PixelClarityDefault` 프로파일은 native scale 1.0, legacy half-height off, dynamic resolution off,
  AA 0, camera/비착석 actor presentation snap을 선언한다. simulation/world transform은 보존한다.
- 활성 도트 1,560개는 기존 Point/no mip/uncompressed/180 PPU를 유지하고 painted UI 대표 4개는
  Bilinear를 유지한다. Sprite Atlas는 없다.
- Unity 6000.3.21f1 Editor render validation, PrototypeValidation, warning 0 Windows x64 Release build와
  D3D11 `1920×1080`, `1600×900`, `1600×1000`, offscreen `2560×1440` 비교 QA가 PASS했다.
  Starter Main Flow도 8방향·충돌·좌석·계약·저장 회귀를 통과했다.
- 상세 감사는 `Docs/RENDER_CLARITY_AUDIT_2026-08-14.md`다. canonical main이 작업 중 전진했으므로
  아직 commit/push/deploy하지 않았고 commander 신호 후 최신 main에서 재적용·최종 빌드한다.
