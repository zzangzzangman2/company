...
끌어들여 사용자 재검수에서 폐기됐다. V24는 책상 겹침을 해결했지만 사용자가 의자를 더 뒤로 빼고 손이
모니터가 아닌 키보드를 치게 해야 한다고 지적했다. V25는 seat와 wrist를 고쳤으나 확대 검수에서
keyboard↔screen 간격이 약 4cm뿐인 원인이 남았다. 최종 격리 build/runtime은
`FatherV19MeshyOnePackage613MapBuildV21ChairBackClearTyping` /
`FatherV19MeshyOnePackage613MapRuntimeV26ChairBackClearTyping`이다. 실제 phase
...
몸 뒤 손과 옆으로 빠진 발은 폐기했다. V26은 카메라용 yaw를 `0°`로 고정하고, 초록 글씨 screen의
실제 앞면 법선 위에 의자·아빠·키보드를 놓는다. screen-front→seat, actor→CRT, chair→CRT,
actor→keyboard는 모두 `0°`다.

V26은 실제 `desk_father`의 정수 origin `(2,8)`, `2×1` footprint, 네 footprint corner, desk seat/work
...
`0.5279419 ≤ 0.5656523`다. 키보드는 물리 desk front 안쪽 `0.1466583`, 좌석은 front 바깥
`0.3812838`이고 요구 최소 clearance는 `0.2639711`이다. keyboard→screen은
`0.1535015 ≥ 0.1319855`다. semantic route seat에서 최종 착석점까지는 `1.0925947`을 부드럽게
...
다른 가족 캐릭터 포팅은
[FAMILY_3D_WORKSTATION_CHARACTER_REUSE_CONTRACT_2026-08-28.md](FAMILY_3D_WORKSTATION_CHARACTER_REUSE_CONTRACT_2026-08-28.md)의
신장 비례 좌표식·수치 gate·숨김 실행·132프레임 검수 순서를 그대로 사용한다.

상점 구매의 정수 타일 snap/footprint/`BlocksMovement`는 기존 production
`OfficeFurnitureTransactionService`/`OfficeLayoutEditRules`가 이미 담당한다. 이번 proof는 3D 표시가
같은 계약을 따르게 한 것이며 구매 직후 chair/seat/workstation binding을 자동 생성하는 production
변경은 사용자 승인 전에는 하지 않는다.
