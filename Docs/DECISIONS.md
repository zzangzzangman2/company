...
- 소스 인계만 승인된 것이며 실제 게임 실행, 그래픽 검증 재개, Release 게시 또는 기존 메인 교체로
  확대하지 않는다. 최신 공개 게임은 v5 그대로이며 미완료 런타임/화면 검증을 PROJECT_STATE에 유지한다.

## 2026-09-07 / 사용자 업무 방해 중단과 화면 없는 검증만 재개

- 사용자가 검증 중 클릭 장애를 보고했다. 정확한 원인은 아직 확정되지 않았다. 사용자 프로그램은
  조작하거나 종료하지 않았으며 확인 당시 QA Player/Unity 프로세스는 이미 남아 있지 않았다.
- 사용자의 후속 지시는 방해 없는 백그라운드 진행이다. private desktop이나 D3D11 batch 렌더를
  무조건 안전하다고 간주하지 않는다. 화면/입력 검증은 새 승인을 받을 때까지 보류한다.
- 공용 QA desktop 런처와 7개 그래픽 스크립트, FastQA 그래픽 프로필에 `-AllowGraphicsQa` 명시
  게이트를 추가했다. 기본 거부는 프로세스 생성 전에 발생하며 검증 생략을 PASS로 바꾸지 않는다.
  공용 런처의 알려진 native-input QA 플래그는 그래픽 승인과 별개로 차단한다.
- warm response file을 사용하는 별도 순수 UI 컴파일러는 Unity/Player를 실행하지 않고 Artifacts만
  쓴다. 낮은 우선순위·최대 2 CPU 코어·직렬 컴파일을 적용한다. 이는 C# 컴파일 증거일 뿐 에셋 임포트,
  Canvas lifecycle 수정의 실제 런타임 검증이나 배포 승인이 아니다.

## 2026-09-07 / 다섯 허브와 상세 화면의 캐주얼 재설계·전체 패치 진행률

- 사용자가 기존 장식형 내부 화면을 거부하고 이미지 생성까지 포함한 전면 캐주얼 UI를 요청했다.
  흰색 HUD만 바꾸었던 앞선 제한을 화면 표시 계층에 한해 확장한다. 맵·가족 3D·가구 형상·게임 규칙은 불변.
- 작은 오피스 일러스트 한 세트를 built-in ImageGen으로 새로 생성한다. 글자·버튼은 native UI이며,
  과도한 금색/코럴 장식 프레임 대신 paper/sage 공통 표면과 기존 OFL Pretendard로 통일한다.
- 실제 viewport 폭으로 카드 너비를 계산하고 긴 콘텐츠는 내부 스크롤로 제공한다. 작은 글씨나
  허용치 완화로 잘림을 숨기지 않는다. 승인되지 않은 기능을 구현 완료처럼 표현하지 않는다.
- 진행률은 시간 추정이 아닌 단계별 가중치다: 전체 압축 수신 bytes 0–85, 전체 파일 검증 bytes 85–99,
  성공 결과 100. 파일·폴더 전환/압축 해제에서 역행하지 않는다. 원시 단계 bytes/percent는 로그에 유지.
- 재시작은 실제 화면 Repaint 뒤 4초 안내가 끝나야 시작한다. 일반 parent 종료·해시 확인·원자적 전환은
  유지한다. batchmode에서 화면이 없어 실패한 관측을 통과시키려고 gate를 낮추지 않는다.
- 고정 v2 설치의 bootstrap은 snapshot 패치만으로 바뀌지 않는다는 기존 구조의 한계를 명시한다.
  사용자 Downloads를 테스트본으로 덮어쓰지 않는다. 실제 첫 화면 수정은 최종 Release 기반의
  승인된 최초 1회 동일 경로 설치 갱신으로 별도 검증해야 한다. 현재 작업은 미배포다.

## 2026-09-07 / 회사 흰색 HUD를 기존 메인의 v5 패치로 배포

- 회사 작업 pull 뒤 미배포 상태를 설명하고 사용자의 `ㄱ` 진행 요청을 받아 받은 UI 그대로 공개한다.
  새 캐릭터·가구·게임 규칙 변경, 새 결제, PC 종료 요청으로 확대하지 않는다.
- 집에서 clean `c5199855`의 정식 Release를 다시 만들고 HUD 4해상도·54개 경로·정상 출근/착석/보행·
  사업/정산·81개 updater 회귀를 새로 검증한다. 기존 상점 native 입력 기록은 바뀌지 않은 구매 코드에만
  연결하며 이번 EventSystem 클릭을 새 OS 마우스 입력이라고 표현하지 않는다.
- `fc-win-20260907.2` / sequence 5 / release 383977367. 게임 SHA와 후속 문서 push SHA를 구분한다.
  기존 Downloads 메인과 사용자 cache/save는 읽기 전용으로 검증하고 실제 패치 전송은 별도 QA root에서 한다.
- Unity의 비배포 Burst 진단 텍스트 1개를 게시 직전 발견하여 publisher만 중단했다(아직 draft 없음).
  exact-root/hash 기록 후 텍스트 폴더만 후보 바깥에 보관했다. 원본 build manifest는 170개 기록을 유지하고,
  실제 배포되는 169개는 검증한 runtime bytes와 모두 같다. 앞으로 후보 목록의 `DoNotShip` 유입도 확인한다.
- 기록 작성 보조 스크립트의 문자열 따옴표 파싱 오류는 실행 전에 수정했다. 게임 검사 실패나 재빌드가 아니며
  검증 PASS를 생성하기 전에 해당 스크립트의 모든 실제 증거 조건을 통과했다.

## 2026-09-07 / 통합 흰색 HUD의 실제 구현·빌드·소스 push

- 사용자의 "빌드하고 push해"는 직전 시안의 통합 상단 바·사무실을 가리지 않는 패널·한 하단 바를
  실제 게임 표시 계층으로 적용하고 검증해 main에 push하는 범위다. 공개 Release 게시와 메인 EXE 교체를
  자동으로 포함하지 않는다. 기존 회사 PC의 아빠 importer 변경과 미완료 누나 후보는 이 커밋에 섞지 않는다.
- 회사 자금과 날짜·시간은 기존 GameState에서 읽는다. 시안의 샘플 금액/진행률을 하드코딩하지 않으며
  사업 패널은 기존 adapter와 업무/배정 기능을 그대로 사용한다. 1×/2×/4× 시간 계약과 pause 메뉴도 유지한다.
- 기본 HUD만 새 흰색 표면·재사용 생성 아이콘·번들 Pretendard로 표현한다. 상세 화면, 월드, 가족 패키지,
  폰트 원본, save와 fixed-main/인게임 패치 경로는 변경하지 않는다. 추가 이미지 생성이나 유료 호출은 없다.
- 후속 "응 백업하고빌드"는 기존 캐릭터 파일 19개의 외부 백업·임시 분리·바이트 단위 복원을 승인한다.
  이를 캐릭터 후보 승격이나 공개 패치 게시 승인으로 확대하지 않는다. 백업과 복원은 SHA256으로 검증한다.
  Unity가 재직렬화하는 새 HUD 이미지 `.meta`의 빈 문자열 뒤 공백은 그대로 추적한다. 공백을 제거하면
  다음 임포트에서 dirty guard가 실패하므로, guard를 완화하지 않고 Unity의 안정된 출력 형식을 커밋한다.

## 2026-09-07 / UI 시안은 실제 맵·승인된 3D 인물을 보존한다

- 사용자는 새로 그린 사무실 시안에서 기존 맵과 3D 인물이 빠진 것을 지적했다. UI 디자인 요청을
  월드·캐릭터 재설계 승인으로 해석하지 않는다. 흰색 UI 바탕과 회사명/자금 한 줄 배치는 유지한다.
- 시안도 실제 릴리스 화면을 불변 바탕으로 사용하고 ImageGen은 UI 아이콘/표면에만 적용한다.
  전체 화면을 다시 생성해 맵·가구·카메라·캐릭터를 대체한 v1–v3 이미지는 월드 정본이 아니다.
- 실제 v4 캡처 위의 표시 계층으로 교정했으며 인물/중앙 맵 픽셀 보존을 확인했다. 아직 Unity 적용이나
  새 UI의 최종 승인이 아니다. 메인 EXE 유지와 인게임 파일 패치/전환 계약은 그대로 둔다.
- 추가 피드백은 회사명·자금부터 날짜·시간·배속까지 분리된 카드가 아닌 하나의 긴 흰색 상단 바와,
  사무실을 가리지 않는 일감 표시다. 시안의 일감 카드는 기본적으로 접고 하단 사업 메뉴에서만 열며
  재클릭·접기·Escape로 닫는다. 좁은 화면은 동일한 한 표면 안에서만 줄바꿈한다. 이 배치 수정은
  기존 그림 자산을 재사용한 UI 시안이며, 전체 미술의 최종 승인이나 런타임 적용으로 해석하지 않는다.

## 2026-09-07 / 첫 사업 루프 v4 공개와 회사 인수인계

- 사용자 승인 범위의 첫 하청→제품→주간 지원을 `c0709823` Release로 검증해 `fc-win-20260907.1`
  (sequence 4, release 383654897)로 공개했다. 동일 소스 main push와 GitHub CI 성공을 확인했다.
- 실제 normal 4인 이동/회피/다음 날 출근/착석, 통제된 8방향 자세, 여러 보행 주기,
  실제 사업 작업·판매·정산과 81개 updater 회귀를 통과했다. 검증 범위 차이는 release evidence에 보존했다.
- 실제 GitHub v2→v4 수신은 16개 160,602,031 bytes, 92개 다운로드 이벤트로 100%,
  153개 재사용/169개 해시 검증을 통과했다. 집 메인·v2 cache·save/backup 5개는 보존했다.
  사용자 메인의 다음 실제 다운로드를 소비하거나 새 Unity 재시작 관측으로 과장하지 않는다.
- 패키지 제작 중 PS5.1 ZIP entry의 역슬래시 문제를 검증기가 차단했다. 해당 ZIP만 기록 후 휴지통으로
  옮기고 PS7에서 생성·동일 파일 해시 검증 후 게시했다. 후속 제작용 publisher에 7.2 이상 guard를 추가하고
  구 host 사전 거부/AST 파싱/draft 조회 7개를 검증했다. Unity 파일/게임 worker는 변하지 않았다.
- 게임 빌드 SHA와 후속 문서·제작 도구 guard SHA를 분리한다. 회사는 동일 메인으로 플레이하고 개발은
  clean main에서 `git pull --ff-only origin main`; 정본 작업 위치와 후속 범위는 COMPANY_HANDOFF_2026-09-07.md.
- 종료는 최종 원격 push 확인 뒤 강제 종료 옵션 없이 요청한다. 사용자 파일/저장을 보호하는 정상 종료이며,
  이 기록을 PC가 이미 꺼졌다는 증거로 사용하지 않는다.

## 2026-09-07 / 타일 경로 마지막 구간에도 동일한 충돌 축 제약 적용

`a307c597` Release 실관측에서 아빠의 40개 Navigating 표본이 타일 중심선에서 최대 0.006725칸
벗어났다(허용 0.0001칸, 충돌 0). 일반 이동은 충돌 투영의 옆 미끄러짐을 막았지만
`TryConsumeExactEndpoint`의 마지막 짧은 구간에는 같은 제약이 없었다. 두 경로에 동일한 방향/축
검사를 적용하고, 막힌 끝점은 정지·재경로 처리한다. 순간 좌표 보정, 충돌 허용치 변경, 통과 판정
완화는 하지 않는다. 실패 후보의 전체 payload는 해시·원시 관측 보존 후 휴지통으로 옮겼다.


## 2026-09-07 / 책상 세트만 있는 초기 게임의 야간 수면 회복

실제 사업 통합 검사에서 장기 진행 차단을 재현했다. 3D 초기 상점에는 정수기·휴게석이 없고 기존
Sleep/OffDuty가 소모 0일 뿐 회복도 0이어서, 가족 전원이 소진된 뒤 일을 완료할 수 없었다.
없는 시설을 추가하거나 체력을 테스트에서 채우지 않는다. 일정상 23:00~07:00의 실제 수면에 한해
8게임시간당 최대 체력 한 바를 정수 비례 회복한다. 일반 OffDuty/사무실 근무에는 지급하지 않는다.
실제 런타임에서는 사무실을 떠난 캐릭터만 Sleep으로 분류한다. 시설 claim이 있는 진행 단계에는
수면 회복을 끼워 넣지 않으며, 미선택 시설 대기만 해제한다. 기존 시설 성공/claim release 회복은 유지한다.
절대 GameTime 경계의 정수 계산으로 1/2/4/17분 분할과 중간 save/restore가 같고, 기존 v12 체력 상태와
profile fingerprint를 보존한다. 이 변경은 첫 사업을 여러 날 플레이하는 데 필요한 진행 차단 수정이다.


## 2026-09-07 / 검증 후 패치 배포·push·종료 승인

- 사용자가 취침 전 추가 질문 없이 검증, 공개 패치 배포, GitHub push, 회사 인수인계와 PC 종료를 명시했다.
  이 승인은 첫 사업 루프의 검증된 변경 배포 범위이며, 새 캐릭터 생성/유료 결제/모델 승격은 포함하지 않는다.
- 기존 누나 후보 albedo의 Default/Standalone 압축만 Uncompressed로 고쳐 기존 전체 import 검사를 통과시킨다.
  PNG/FBX/material/production roster는 그대로다. 해당 후보는 계속 미승격이다.
- 사업 통합은 실제 첫 하청 4인시와 준비된 개발 체크포인트의 마지막 4인시/지원 2인시를 실제 업무로 확인한다.
  체크포인트의 과거 하청/개발 20인시는 순수 코어로 준비하고 주간 정산은 명시적 시간 진행을 사용한다.
  이를 일주일 전체의 무중단 native 플레이로 보고하지 않는다. 네 가족/보행/착석 검사는 별도로 진행한다.
- 사용자 고정 메인·저장·캐시는 보존하고 공개 전송 검증은 별도 QA 저장소에서 수행한다.
  종료 직전 회사용 MD, 원격 main 일치, 공개 Release/자산 확인을 기록한다. 실패한 후보는 공개하지 않는다.

## 2026-09-06 / 첫 하청에서 자체 제품·주간 유지보수까지 먼저 구현

- 사용자의 콘텐츠 기획 후 "진행시켜" 요청은 첫 플레이 루프 구현으로 진행한다. 주식시장 전체 개편이나
  실존 기업의 IP 양도·M&A·새 유료 3D 생성·공개 패치 배포를 함께 승인받은 것으로 해석하지 않는다.
- 기존 단어 DB/대여점 관리 하청을 입문으로 고정 안내하고, 완료 경험을 자체 관리 프로그램으로 잇는다.
  개발비 30만 원/24인시/30일, 시험 3고객/사용권 6만 원, 주간 유지보수 2인시/고객당 2만 원은 잠정 밸런스다.
  사용자 확정 시작 자금 500만 원/책상 세트 40만 원은 변경하지 않는다.
- 새 작업용 AI를 만들지 않고 기존 좌석/이동/업무 coordinator를 재사용한다. 입문은 PC만 사용하는 업무로
  정의하여 상점에 없는 프린터/회의탁자 때문에 끝낼 수 없는 구간을 막는다. 캐릭터·책상 외형은 건드리지 않는다.
- 기술 지급을 player UI가 아닌 공통 계약 완료 정산으로 이동한다. 실제 NPC/E 작업 모두 한 번만 지급한다.
  수락 기술 속도/품질 보너스 및 신규 완료 품질을 스냅샷으로 고정하고 Save v12로 이관한다.
- 내부 개발/지원은 외부 하청과 구분하고 외부 기술/평판/실적을 부풀리지 않는다. 2개 업무 슬롯은 공유한다.
  판매 버튼 반복/로드/시간 분할로 돈이 재발행되지 않도록 고유 정산 ID와 정산 주기를 저장한다.
- 자체 개발/지원은 입문 DB·관리도구 기술을 재사용해 속도/품질 이점을 얻지만 완료 기술점은 추가 지급하지 않는다.
- 실제 업무 검증에서 재현한 인접 타일 몸체 막힘은 동적 swept-body 경로 검사로 수정한다. 업무 배정의 초기
  경로도 동적 점유를 피하며 정지 상대의 stale intent를 이동 우선권으로 보지 않는다. 충돌 크기/중앙 축/외형은
  변경하지 않는다. 출근·이동·착석 준비 시간을 남은 업무에서 차감하지 않는 시계 경계도 명시한다.
- 구체 계약·UI·코드 경계·검증 제한은 `STARTER_BUSINESS_LOOP.md`. 공개판은 계속 `fc-win-20260906.3`이며
  새 콘텐츠는 출시 검증과 별도 배포 승인 전이다. 사용자 메인과 기존 세이브를 FastQA로 덮어쓰지 않는다.
...
- 사용자가 CRT의 기울어 보이는 자세를 지적했다. 실제 overlay에서 CRT/key에만 별도 직각 축을 적용한
  코드가 책상 축 대비 19.4712도 불일치를 만들었다. CRT 면의 공유 corner normal도 표면을 둥글게 보이게 했다.
- 모든 책상 위 직사각형 부품은 책상과 같은 투영 기저를 쓴다. physical authoring의 직각 기저와 현재
  runtime overlay의 mapped coordinates를 혼동하지 않는다. 배우용 카메라/몸체를 이 문제 때문에 바꾸지 않는다.
- CRT 면 normal만 분리하고 dimensions/material/중심/높이는 유지한다. 키보드 상세 키와 화면 글줄도 같은
  기저로 배치한다. 의미 좌석/손목 목표/가구 footprint/의자/보행/세이브는 그대로다.
- source push와 게임 patch 공개를 구분한다. 사용자가 직접 업데이트를 보겠다고 했으므로 기존 고정 메인과
  사용자의 설치 cache는 선제 교체/업데이트하지 않는다. 사용자가 수정 sheet를 보고 "1" / "이 모습으로 배포"로
  승인했다. 공개 이전 독립 Release 검증은 그대로 필요하다.
...
- 실제 후속 검사에서 139개 손 오차가 전부 다음날 재입장 이후에만 발생했다. 출퇴근 표시를 위해
  Host를 deactivate하면 보행 graph/pose handler가 파괴되고 재활성화 시 다시 기준 자세를 캡처한다.
  출퇴근 숨김은 renderer 표시만 전환하여 승인된 rig/calibration 수명을 유지한다. 실제 occupancy의
  Outside/present 상태와 충돌 규칙은 변경하지 않는다.
- 사용자 화면을 건드리지 말라는 지시를 유지한다. QA 소유 프로세스는 전환 권한 없는 별도 Windows
  desktop과 kill-on-close job에서만 실행한다. 바탕화면 전환·전면 창·전역 입력 API는 사용하지 않는다.
- 출근한 숨김 인물의 최초 위치만 문 밖 동일 축의 안전한 대기열에서 찾는다. 고정 한 점의 이전 인물이
  가속하는 동안 다음 출근 시각을 미루지 않으며, 실제 몸 반경과 이동 충돌 검사는 완전히 유지한다.
- 승인된 아빠의 가로 비율 0.92를 변경하지 않는다. 비균등 상위 scale 아래에서 world-space 길이로
  풀던 착석 IK를 parent metric으로 교정한다. 뼈 길이·위치·공유 보행 clip은 바꾸지 않는다.
- source/Editor 회귀 통과는 게임 배포 승인이 아니다. 실제 normal Player 및 현재 시각 승인 gate를
  통과하지 않은 상태로 Downloads를 교체하거나 public Release receipt를 만들지 않는다.

## 2026-09-06 / 집·회사 고정 메인 EXE와 최신 공개판 필수 확인

- 사용자가 같은 메인 파일을 계속 실행하고 회사에서도 매번 빌드하지 않도록 명시했다. 공통 진입점은
  `%USERPROFILE%\Downloads\FamilyCompany_Playtest\FamilyCompany.exe` 하나이며 정확한 절차는
  `MAIN_GAME_ENTRY.md`가 소유한다. 플레이 PC에는 최초 검증된 전체 패키지만 한 번 설치하고 이후에는
  게임 내부에서 변경 파일 다운로드·해시 검증·snapshot 전환/재시작을 수행한다.
- 최신 정식 Release 확인 실패 때 이전 설치본이 온전해도 실행하지 않는다. 기존 오프라인 실행 버튼과
  worker fallback을 제거했다. 재시도/종료만 제공하며 source push를 게임 Release로 간주하지 않는다.
- 집의 고정 경로를 읽기 전용 검사한 결과 실제 파일은 2026-08-18의 `9144fa0e` 구버전이며 worker가
  없다. 이를 최신 자동패치판이라고 안내하거나 미검증 QA로 덮어쓰지 않는다. 첫 Release 공개/설치가
  아직 남아 있다. 사용자 main·Downloads·세이브는 변경하지 않았다.

## 2026-09-06 / 출입구의 개별 진입 통로와 실내 타일 예약 분리

- 후속 실게임에서 앞사람이 뒤따르는 진입자에게 이름 순서 때문에 양보해 다른 통로로 후퇴하는 것도
  확인했다. 상대에게서 빠져나가는 선행자/접근하는 후행자 관계에서는 선행자가 먼저 나간다. 교차·마주침의
  동률에는 기존 이름 순서 유지. 기록 좌표 독립 fixture와 기존 navigation/gait는 통과했지만 마지막
  Player 뒤에 고친 소스라 normal Player 재검증은 남아 있다.
- 출근 대기열만 0.05초 간격으로 재시도하고 0.35초 cooldown은 실제 경과 시간으로 차감한다.
  이 수정만으로 Mother 09:04 지연이 해결되지는 않았다. 관측 종료를 09:50으로 늘린 것은 진단 데이터
  수집이며 원래 09:20 실패를 통과로 바꾸지 않는다. 첫 실제 착석 시각도 actor별 기록한다.
- 마지막 테스트 종료에서 window-handle 안전장치가 발생했으므로 즉시 해당 소유 프로세스를 중단하고
  전면 재실행하지 않는다. 이후 Player 검증은 보류하고 소스/증거만 인계한다. guard가 예외를 내도
  finally에서 실패 원인·실제 exit/강제 종료 여부를 기록하도록 runner를 보완한다.
- 단일 소유자가 2.5칸 전체 통로를 다 지날 때까지 다음 출근자를 숨기던 구현을 개별 통로 예약으로
  바꾼다. 출발점의 실제 몸체 간격을 확인한 뒤 순서대로 진입하고, 진행 중 swept body collision은
  그대로 유지한다. 네 캐릭터의 속도·보폭·크기·충돌 반경이나 출근 시각을 바꾸지 않는다.
- 첫 이동 축은 책상까지 가는 경로의 두 번째 점이 아니라 실제 문 `(8,0)`에서 실내 `(8,1)`로 향하는
  축이다. 이후 각 책상으로의 정상 경로와 독립적이다.
- 실게임 첫 검증에서 몸체는 떨어져 있지만 반올림한 타일이 같아 앞사람까지 멈추는 문제가 재현됐다.
  진입 중인 사람은 자신의 통로/capsule을 점유하되 실내 타일 전체를 예약하지 않는다. 실내 일반
  경로 예약만 이 구분을 사용하며 서로의 몸체를 통과할 수는 없다. 독립 rounded-cell fixture의
  수정 전 실패와 수정 후 통과를 모두 보존한다. 실제 다음 날 관측 결과는 PROJECT_STATE를 따른다.
- Working 의미 상태와 0.42초 시각 착석 blend를 구분해 계측한다. 상태 이름만 보고 전환 중 손 위치를
  완성 포즈 오류라고 단정하지 않는다. 연속 원본 CSV에 blend·양손 오차·양쪽 무릎·ready를 추가하고,
  전환 표본도 보존한다. 이 관측은 pose를 바꾸지 않으며 피부 관통/발 미끄러짐/시각 승인을 대체하지 않는다.

## 2026-09-06 / 일반 보행의 타일 레일과 좌석 접근 권한 분리

- 빈 사무실 산책뿐 아니라 가구 배치 후 일반 자율 경로에도 타일 중심선 후퇴·재탐색을 적용한다.
  옆으로 밀어내는 벡터가 경로 제약에 의해 0으로 투영되던 회복 분기를 제거하고, 속도 0의 교통 대기도
  막힘 시간에 포함한다. 이동 중인 상대의 순서 우선권은 유지하되 가만히 있는 수동 Player/착석자를
  기다리기만 하지 않고 경로가 허용하는 타일 레일로 후퇴한다. 모델/반경/3칸 점유는 바꾸지 않는다.
- 좌석 예약은 일반 보행자가 자기 의자를 관통할 권한이 아니다. 목적지가 좌석 앞 접근 칸이면 경로 탐색,
  보행 collision, 정확한 endpoint 소비와 양보 후퇴 모두 일반 가구 충돌을 적용한다. 실제 착석/퇴석
  전환의 좌석 권한은 유지한다. 4회전 × 472개 접근 경로 중 수정 전 137개 관통, 수정 후 0개.
- 창업일 가구 배치 뒤 Player는 기존 수동 조작 대상이다. 입력 없이 NPC와 함께 자동으로 일하라고
  강제하거나 구매하지 않은 편의시설에 대한 원래 루틴을 바꿔 PASS를 만들지 않는다. 정상 보행,
  NPC 업무 표본, 다음 날 정시 출근·착석, 네 방향 포즈, native shop/UI는 각각 별도 결과로 기록한다.
- 상점 일시정지/배치 rebuild 구간은 정상 보행의 정지시간이 아니다. 독립 CSV 분석에서 실제 상점
  setup clock과 ready/shopOpen을 제외하며, 수정 전후 같은 규칙을 적용한다. 첫 회 통과 이후 반복
  충돌 실패가 나오면 통과로 덮지 않고 해당 실행 payload 전체를 증거 보존 후 회수한다.
- 글씨 검증은 실제 게임 글꼴·스타일의 5개 높이별 75개 영역 측정으로 자동화한다. 사용자 화면을
  조작하지 않으며 이 측정을 IMGUI 픽셀 검수나 사용자 시각 승인으로 바꾸어 보고하지 않는다.
- c2644f61의 정상 경로 독립 분석은 관통 0/최대 보행 대기 1.0169초로 통과했지만 다음 날 정시
  입장과 09:20 전원 착석은 실패했다. 단일 출입구의 2.5칸 진입 예약을 이동 속도 상승·충돌 무시로
  우회하지 않는다. 오전 전체를 관측하되 실패를 유지하고 실행 payload를 회수한다. Working 전환
  손 위치의 3개 outlier도 원본 CSV에 보존하며 isolated fitting PASS로 연속 동작을 승인하지 않는다.

## 2026-09-06 / 네 방향 의자 바닥 중심은 좌석 타일 중앙

- 사용자 추가 지시: 이후 검증은 백그라운드 전용. 바탕화면/마우스/키보드 조작과 전면 게임 실행 금지.
  `-batchmode -force-d3d11`와 offscreen camera stack만 사용하며 IMGUI 화면 확인으로 보고하지 않는다.
- 중앙 고정 후 아빠의 손 도달성이 부족해, 의자/골반/다리/뼈 길이를 유지하고 spine만 필요한 만큼
  최대 35° 기울인다. 6ce5e0eb에서 두 몸체 × 네 방향 실제 손 endpoint·무릎·쿠션 관통 측정 PASS.
  이 검사는 직접 포즈 호출이며 자율 착석/타이핑 영상이나 사용자 승인으로 대체하지 않는다.
- 책상은 기존 2칸 안에서 깊이 86% 대신 100%를 사용하고 키보드를 앞줄 끝 안쪽에 놓는다.
  녹색 점유 3칸을 늘리거나 의자를 이동하지 않는다. 짧은 다리는 실제 허벅지/정강이 길이로
  95° 목표 무릎에 필요한 발목 높이를 계산하되 기존 높이보다 내리지 않아 의자 바닥판을 피한다.
  32개 조합 검사에서 골반 높이를 올리지 않는 `0.113h/95°`가 두 모델 모두 손 오차/쿠션 관통을
  통과했다. 최종 8방향 조합도 관통 0, 최대 개별 손목 오차 0.0081 world,
  실제 무릎 Player 약 95°, Father 99.18°/99.65°를 확인했다(비등방 스케일로 목표와 차이 가능).

- 사용자 지시에 따라 의자 기둥/바닥판의 지면 중심을 semantic seat.Cell 중앙에 고정한다.
  높이가 포함된 sprite operator socket을 바닥에 투영하거나 키보드에서 일정 거리만큼 의자를
  끌어오는 구형 보정은 production에서 사용하지 않는다. 네 회전과 구매 미리보기에 같은 계약 적용.
- 모니터/키보드는 의자와 다음 책상 타일의 공통 축에 놓고, 모니터 면은 의자를 바라본다.
  의자의 부품 크기/모양, 배우 크기/보행과 무릎 허용 범위는 유지한다. 반 타일 간격을 없애기 위해 의자를
  옮기지 않는다. 새 착석 팔 도달성은 실제 Avatar 손 위치로 별도 측정해야 하며 기하 테스트로 대체 못 한다.
- 상세창의 상품명/가격/잔액/배치 오류/방향을 한국어와 독립된 줄에 배치하고 버튼 높이를 예약한다.
- 원래 main EXE를 계속 열고 게임 내부에서 패치를 확인하는 구조 유지. 로컬 UI/worker 테스트와
  GitHub 배포는 별도 게이트이며, 미검증 Release를 올리지 않는다. 실제 Unity 재시작은 로컬 전송으로
  별도 검증했으며 정상 게임 플레이/인터넷 패치 성공과 혼동하지 않는다.

## 2026-09-06 / 상점 문구와 버튼 영역 분리

- 사용자가 상품명과 구매 버튼의 겹침을 지적했다. 판매 카드의 제목을 `책상·PC·의자 세트`로 통일하고,
  가격은 `400,000원`, 점유는 `3칸 점유`로 표시한다. 사용자용 카드에서 개발 분류명 `WorkDesk`는 빼며,
  기존 메이플스토리 글꼴/크림·청록 스타일은 유지한다. 텍스트를 고정된 버튼 열에 넘겨 그리지 않는다.
- 가격·모델·3칸 footprint·회전·착석 로직 변경은 없다. 검증 전 릴리스 승격도 하지 않는다.
- 미추적 누나 V2 입력은 사용자 continuation 후 원본 해시 그대로 보존 커밋만 했다. 실제 생성/승격 권한이 아니다.
- 독립 수동 gameplay 관측은 명시적 진단 인자로만 켜며 네트워크 패치를 건너뛴다는 사실을 기록한다.
  이는 공개 전 게임 검증용이며 일반 실행/실제 패치 성공 증거와 분리한다.

## 2026-09-06 / 사용자 정정: 별도 창 금지, 게임 내부에서 패치

- `아니 이렇게말고 게임안에서패치`가 아래 외부 런처 결정을 대체한다. WinForms 로딩창과 그 빌드/테스트
  컴파일 진입은 제거한다. 메인 Unity 게임의 기존 UiRemasterV3 로딩 배경/패널을 그대로 사용한다.
- 시작 시 다운로드 바이트 이벤트를 숨김 worker에서 받아 게임에 표시하고, 타이틀 입력/사무실 준비는
  패치 확인 뒤에 연다. 실행 중 EXE를 덮지 않고 별도 snapshot을 검증한 다음 정확한 부모 종료를 기다려
  적용/재시작한다. 첫 설치 ZIP에도 실제 Unity 게임이 들어가야 한다.
- 실제 게임 화면 20.3% / 0.81 of 4.00 MiB를 확인했다. 화면 없는 batch 모드의 검은 PNG는 폐기된
  시각 증거이며 앞으로 픽셀 밝기 검사를 통과한 presented frame만 사용한다. 숨김 실행 원칙상 이 UI
  직접 검증은 사용자에게 먼저 알린 뒤 명시적 ShowWindow로만 한다.
- Core 51개 로컬 검사는 PASS. 실제 GitHub 게임 배포/재시작 및 남은 independent gameplay gates는
  아직 완료가 아니다. 누나 입력 13개도 답변 없이 stage/이동하지 않는다.

## 2026-09-06 / 폐기된 외부 창 시도 (바로 위 사용자 정정이 우선)

- 사용자의 `괜찮아 패치배포하고 ... 로딩창 ... 정확히`를 최신 기본 4인 산책 영상의 시각 승인으로 기록한다.
  그 문장이 아직 하지 않은 착석 4방향/상점 native 클릭/다음 날 출근/mute의 PASS를 만드는 것은 아니다.
- 배포 진입은 `FamilyCompany.exe` 네이티브 x64 런처로 한다. Unity payload의 동명 EXE는 검증된
  `%LOCALAPPDATA%/FamilyCompany/PatchedGame/versions/...` 안에 있으며 직접 실행 진입으로 안내하지 않는다.
  RUN_WINDOWS도 새 런처로 연결하고, 기존 Downloads 실행본은 실제 Release 검증 전 교체하지 않는다.
- 진행률은 변경된 압축 파일의 총 바이트 대비 실제 수신 바이트로만 계산하고 소수 1자리에서 내림한다.
  파일 개수나 타이머로 만든 전체 퍼센트는 사용하지 않는다. 다운로드/해제/검증/활성화를 구분한다.
- 네트워크 실패 때 기존 설치도 다시 해시 검증한 뒤 사용자가 이전 버전 실행을 선택하게 한다.
  취소는 협력식이며 진행 중 I/O 응답에 최대 30초가 걸릴 수 있다. 파일 잠금/세이브/원자 pointer 계약을 유지한다.
- 로컬 스트림 네이티브 UI와 48개 회귀 테스트는 통과했다. GitHub에는 아직 game Release가 없으므로
  실게임 첫 설치/변경 패치/네트워크 복구의 완료 증거로 바꾸지 않는다. 검증 스킬 `computer-use`로 실제
  표시를 확인했고, 이때 발견한 worker 해시 호환성 문제도 .NET SHA-256 및 x64 빌드로 수정했다.

## 2026-09-05 / 실패 QA 직접 정리, 보이는 가구 여유와 타일 예약 교착 회복

- 사용자의 `너가해` 재승인에 따라 검증된 확인용 WindowsPlayer 한 경로만 휴지통으로 옮겼다.
  각 실패 identity별 166개 파일 해시와 실패 원인을 바깥에 기록하고 target=0을 확인했다.
  Library/Bee, 저장 데이터, source, 누나의 기존 untracked 입력은 보존한다.
- 기본 아들/아빠는 승인 외형을 바꾸지 않고 동적 반경 0.445/0.415를 사용한다. 정면 84픽셀 겹침은
  새 실행본에서 0이 됐다. 기본 가구 패딩도 0.18과 기존 인접 칸 비용 +2.5를 적용해 책상 관통
  11/14프레임을 0/0으로 줄였다. 본인 좌석/책상 접근 예외와 구매 점유칸은 바꾸지 않는다.
- 빈 사무실에서 서로 예약한 타일 때문에 오래 멈추는 경우, 실제 예약 소유자까지 보고 낮은 우선순위
  배우만 현재 타일 중심선 위의 안전한 중심으로 물러난다. 기존 목적지는 유지하고 이후 정상 경로를
  재계산한다. 충돌 허용, 순간이동, 중심선 밖 비스듬한 밀어내기로 교착을 숨기지 않는다.
- 60초 정상 산책에서 8초 이상 진행이 없는 배우가 있으면 실패한다. 새 실행에서 네 명 모두 50~54
  world units 이동했고 최대 무진행 1.9206초, 충돌 위반/오류 0이었다. 과거 self-PASS는 현재 근거가 아니다.
- pair fixture가 남기는 오래된 `releasePlayer=true`/`productionEligible=True` 문구는 승인서가 아니다.
  외부 실행기는 EXE+게임 DLL hash, 실제 FastQA provenance와 `productionEligible=false`를 따로 남기며
  관통 계측이 빠지거나 양수이면 실패한다. 독립 release gate/사용자 승인은 여전히 필요하다.

## 2026-09-05 / 타일 중심 경로와 개발 설정 즉시 반영, GitHub 파일 단위 패치

- 사용자가 기존 GitHub 배포를 선택했다. 일반 사용자는 검증된 Release를 런처로 받고, 개발자는
  FAST_QA + 명시적 JSON live reload로 숫자를 조정한다. C# 컴파일 자체를 없앤다고 약속하지 않는다.
- Resources 기반 현재 패킹을 유지하므로 변경된 내장 리소스 파일은 그 파일 전체를 받는다.
  Addressables 전환이나 에셋별 원격 스트리밍을 구현한 것으로 표기하지 않는다.
- GitHub 저장소/태그/자산 경로를 고정하고 원본·압축 SHA-256, 임시 snapshot 검증 및 current
  pointer 원자 교체를 쓴다. 임의 URL/절대 경로/디렉터리 탈출/접합점/동일 버전 변조를 거부한다.
  서버의 asset digest와 TLS가 신뢰 경계이며 별도의 코드서명이 아니다. 토큰은 런처에 넣지 않는다.
- 기본 모델 스케일/색/클립은 보존하고 보행 경로의 시작 중심 누락과 코너 관성만 수정했다.
  고정된 주기 평균 발 중심 오프셋과 아빠 접지 높이를 적용했다. 이후 정면 충돌 화면에서
  84픽셀 겹침이 발견되었다. 이후 반경 보정/실행 검증 결과는 위 후속 결정이 최신이다.
- 사용자 승인/독립 release gate 없는 FastQA는 공개하지 않는다. 실패 실행본 정리 명령이
  당시 도구 정책에 차단됐으나 이후 사용자가 직접 처리를 재승인하여 해결했다. 완료 전 PC 종료는 실행하지 않았다.

## 2026-09-05 / 빈 사무실 4인 시작, 임시 3D 외형과 단일 40만 원 세트 상점

사용자가 엄마·누나의 모델 제작은 미루고 아들·아빠 외형을 한 명씩 더 써서 네 명 모두 3D로
돌아다니도록 요청했다. 시작 자금 **500만 원**, 책상·PC·의자 **한 세트 40만 원**도 직접 확정했다.

- 새 게임은 무료 책상·좌석 없이 시작한다. 창업 첫날 08:50에는 네 가족이 이미 실내에 있고 실제
  coordinator의 기존 빈 사무실 자율 산책을 사용한다. 시뮬레이션의 09:00 업무 시작과 이후 날짜의
  정상 출퇴근 규칙은 유지한다. 시간을 강제로 앞당기거나 목적지를 QA 코드로 주입하지 않는다.
- `OfficeFamily3DVisualRoster`만 외형을 매핑한다: `older_sister -> PlayerV8`, `mother -> FatherV19`.
  각자의 semantic ID/역할/나이/save/예약/좌석은 유지하며 renderer·Humanoid·보행 인스턴스는 별개다.
  원본 모델/클립/스케일/색/가구 및 후보 프로필 승격 상태는 바꾸지 않는다. 엄마·누나의 신규 3D
  패키지를 생성한 것이 아니며, 기존 V2 입력 작업과 generation gate를 보존한다.
- 상점 공개 상품은 승인 V31 세트 하나다. 기준 가격 desk 1,400,000 + chair 200,000에 기존
  gameplay 25%를 적용해 350,000 + 50,000 = 400,000원. 기존 save의 구매 원가·재판매 근거는 수정하지
  않는다. 다른 가구 정의/legacy transaction은 이전 save와 fixture용으로 남기되 상점 구매에서는 숨긴다.
- 실제 점유는 책상 2칸 + 의자 1칸이며, 타일 기준 4방향 강체 회전/접근칸/충돌/자리 배정과
  preview -> confirm -> 한 번 차감 규칙을 유지한다. 4세트 차감은 총 1,600,000원이다.
- 새 opt-in `Family3DOpeningShopQa`는 정상 60초 산책, 4개 독립 Humanoid, 2D 숨김, 단일 상품,
  4방향 preview/실제 controller confirmation/잔액/겹침 거부를 계측한다. 숨김 D3D11 실행이므로
  programmatic confirmation 결과를 native pointer 클릭 증거로 부르지 않는다.
- 배치 리빌드 재검증 중 발견한 stale reference도 수정한다. `IOfficeRuntimeAgent` 인터페이스의
  `!= null`은 Unity에서 Destroy된 컴포넌트를 걸러내지 않아 `ResetAssignments -> ResumeAutonomy`가
  이미 제거된 Grid/Transform에 접근했다. 해당 실제 Unity 객체의 null 수명을 확인해 파괴된 대상만
  건너뛴다(살아 있는 에이전트와 순수 C# 대역은 그대로 처리). 3D presenter도 부분적으로 무효해진
  binding을 재생성 전에 해제한다. QA는 화면/가격 assertion뿐 아니라 runtime Error/Exception이 한 건이라도
  있으면 FAIL로 종료하며, 실행기에서 최종 로그까지 별도로 검사한다.
...
중심에서 Player `0.514`, Father `0.407` 월드 단위까지 뻗는데 가구 회피 반경은 `0.22`였고, 격자 한 칸의
모서리 수직 폭은 약 `0.79`(반칸 `0.397`)다. 책상 옆 칸을 붙어 걸으면 팔이 책상 상판 안으로 들어간다
(우회 테스트 113프레임 중 `36/68`프레임에서 skinned 정점이 책상 부품 박스 내부).

결정: 후보 전용으로 두 규칙을 넣는다. (1) `OfficeRuntimeOccupancy`에 배우별 **가구 전용** 여유 패딩
`+0.18`(합계 `0.40`). 벽·바닥 셀에는 적용하지 않아 벽 옆 줄은 그대로 걷고, 본인 좌석 책상은 기존
permitted 예외로 착석에 영향이 없다. 반칸 `0.397`을 넘기면 책상 옆 칸 자체가 막혀 정체가 생기므로(`0.49`
시험에서 700프레임 정체 확인) 그 아래로 둔다. (2) `OfficeRuntimePathService`에서 패딩이 있는 배우는 책상
footprint에 접한 칸에 `+2.5` 비용을 줘 여유가 있으면 한 칸 떨어져 우회하고, 목표가 책상 옆이면 그대로
도달한다. production/default는 패딩 `0`, 비용 `0`으로 불변.

결과 `Artifacts/FatherDeskClearanceFinal-20260902-160500/`: 우회 경로가 `(1,x)` 열·`(x,10)` 행으로
바뀌고, 책상 부품 로컬 박스 기준 정점 침투 `0/0`프레임(이전 `36/68`), 최근접 정점 거리 `0.547/0.199`
월드, 정적/상호작용 위반 `0/0`, 착석 `Working/Working` 유지. 측정은 월드 AABB가 아니라 부품 로컬 박스로
한다(격자가 월드 축에 대해 기울어져 AABB는 과대 판정).
...
Player `0.28`, Father `0.30`이며 `OfficeRuntimeOccupancy`의 peer 충돌·예약·침범 측정에만 사용한다.
...
`1.769311871`로 고정했다. 전체 면적만으로는 큰 머리를 숨길 수 있으므로 가로축에는 별도 `0.92`
proportion scale을 적용한다. 1280x720 QA는 머리 폭 Player/Father `22/22px`, 몸통 폭 `30/29px`,
전체 높이 `74/72px`, 면적 차이 `7.78%`를 기록하며 머리·몸통 폭 차이 1px 이하를 강제한다.
잘못된 동적 환산값 `1.437899/2.677095`와 여전히 크게 보인 중간값
`1.012728333/1.885507822`는 production에서 폐기한다.

검증: Unity `6000.3.21f1` D3D11에서 정면 동시 접근 시 두 사람 모두 이동했고 agent block `45`,
