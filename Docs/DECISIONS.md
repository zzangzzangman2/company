...
## 2026-08-10 / 플레이어의 월드 조작 말 외형 고정

결정: 기존 `simul` 타이틀의 14살 소년 디자인을 가족회사 플레이어의 런타임 조작 말로 사용한다. 빨간 뉴스보이캡, 짧은 짙은 갈색 머리, 흰 후드 윈드브레이커와 줄무늬 티셔츠를 4방향 2프레임 도트에서 고정한다. 이는 별도 VN 초상화나 실존 사용자의 얼굴을 정의하는 결정이 아니다.
