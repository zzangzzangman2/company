...
- 중앙 고정 후 아빠의 손 도달성이 부족해, 의자/골반/다리/뼈 길이를 유지하고 spine만 필요한 만큼
  최대 35° 기울이는 fitting을 시험한다. 네 방향 손 endpoint·무릎·쿠션 관통 독립 측정 전 승인 아님.
- 책상은 기존 2칸 안에서 깊이 86% 대신 100%를 사용하고 키보드를 앞줄 끝 안쪽에 놓는다.
  ...
  통과했다. 네 방향 최종 검사를 별도로 수행한다(비등방 스케일 때문에 실제 무릎 각은 목표와 다를 수 있다).
...
- 모니터/키보드는 의자와 다음 책상 타일의 공통 축에 놓고, 모니터 면은 의자를 바라본다.
  의자의 부품 크기/모양, 배우 크기/보행/무릎 표준은 유지한다. 반 타일 간격을 없애기 위해 의자를
...
- 원래 main EXE를 계속 열고 게임 내부에서 패치를 확인하는 구조 유지. 로컬 UI/worker 테스트와
  GitHub 배포·실제 Unity 재시작은 별도 게이트이며, 미검증 Release를 올리지 않는다.
