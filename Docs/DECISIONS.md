...
결정: 타일 사무실의 첫 가구는 잘못 잘린 구형 4×3 아틀라스를 재사용하지 않고, OpenAI 내장 ImageGen으로 한 이미지에 한 소품만 담은 12종을 새로 생성한다. `OfficeGrid`가 18개 배치와 4개 좌석의 의미 상태를 소유하고, Unity Presenter는 같은 x+y 깊이 규칙으로 가구·가족을 투영한다. 의자는 통행 가능한 좌석 셀이며, 등받이만 별도 전경 레이어로 분리해 착석 인물을 자연스럽게 가린다. 이 작업은 `OfficeTileMigrationPreview`에 격리하고 기존 `Prototype01`의 계약·자율 AI·A*와 아직 결합하지 않는다.
