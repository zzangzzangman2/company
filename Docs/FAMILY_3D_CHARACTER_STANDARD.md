...
6ce5e0eb의 중심 고정 계약은 책상 깊이를 기존 점유 타일의 100%로 사용하고, 키보드 앞줄을
`frontForward + depth/2 + 0.002h`로 둔다. 아래 표의 구형 `0.020h / ≤0.30h`는 historical fitting에만
해당한다. production 팔 fitting은 골반/의자를 이동하지 않고 실제 양팔 길이로 필요한 spine lean을
찾아 최대 35°로 제한한다. 발의 X/Z와 최소 높이 `0.158h`는 유지하고, 허벅지/정강이 실제 길이와
95° 목표 무릎으로 필요한 발목 높이를 계산한다. 짧은 체형은 다리를 뻗는 대신 발이 자연스럽게 뜰 수 있다.
최종 직접 포즈 검사 8건에서 실제 무릎 Player 약 95°, Father 99.18°/99.65°, 최대 개별 손목 오차
0.0081 world, 의자 모든 부품 안의 skin 정점 0. 근거는 `Evidence/ChairTileCentre20260906/`.
이는 한 시점의 pose-injection 검사이며 정상 접근/연속 타이핑/사용자 승인/Release 적격 증거가 아니다.
...
| 무릎 각 | 네 방향 모두 `80°~140°`; 현행 중앙 고정 fit의 실측은 위 문단, 구형 fit은 Player `107/113°`, Father `96/100°` |
