...
- `b397af9`의 사무실 관리 구매 클릭과 빈 사무실 자율 산책 회귀는 `a9c6885e`에서 수정되었고 실제 native
  ...
  4인 구성은 사용자 승인 후 공개되었으며 현재 최신은 캐주얼 UI·전체 진행률 패치 `fc-win-20260907.3`다.
  회사 인수인계는 `Docs/COMPANY_HANDOFF_2026-09-07.md`; 게임 소스는 `cb86605d`이며 이후 문서 SHA와 구분한다.
  같은 메인 경로를 유지한다. 집 설치는 사용자 승인 후 전체 백업하고 v6로 갱신했다. 엄마/누나의 고유 3D 모델은 추후 작업이다.
